...
for i, x in ipairs(modes) do
    ...
    log[x.name] = function(...)
        ...
        local server_name = skynet.getenv("svr_id") or ""
        -- print("_______server_name___",server_name)
        str = string.format("[:%08x] " ..server_name .. format_str .. "\n", skynet.self(), nameupper, os.date("%m-%d %X"), lineinfo, msg_str)
        ...
        -- print("____1111111111_______send__",x.name,file,str)
        local ok, result = x_pcall(cluster.send,'loggerserver','.proxy',x.name,file,str)
        if not ok then
            skynet.error("_____Log.lua____")
